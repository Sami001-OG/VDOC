# `vdoc.providers.ocr.paddle`

PaddleOCR provider.

## Classes

### `OCRProvider(config: Optional[ProviderConfig] = None)`
...
- `close()` &mdash; 
- `extract_batch(image_paths: List[Path])` &mdash; Extract text from multiple images.
- `extract_text(image_path: Path)` &mdash; Extract text from an image.
- `get_metadata()` &mdash; 
- `health_check()` &mdash; 
- `initialize()` &mdash; 
...
### `PaddleOCRProvider(config: Optional[ProviderConfig] = None)`
