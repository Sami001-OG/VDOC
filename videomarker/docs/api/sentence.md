# `vdoc.providers.embedding.sentence`

Sentence-transformers embedding provider.

## Classes

### `EmbeddingProvider(config: Optional[ProviderConfig] = None)`
...
- `close()` &mdash; 
- `embed(texts: List[str])` &mdash; Generate embeddings for a list of texts.
- `embed_query(text: str)` &mdash; Generate embedding for a search query.
- `get_metadata()` &mdash; 
- `health_check()` &mdash; 
- `initialize()` &mdash; 
...
### `SentenceEmbeddingProvider(config: Optional[ProviderConfig] = None)`
